define(["iris", "d3"], function (Iris, d3) {
    ...
    Iris.Renderer.extend({
        ...
        exampleData: function() {
            return [["bar a", 5], ["bar b", 4], ["bar c", 3], ["bar d", 7], ["bar e", 1], ["bar f", 4]];
        },
        render: function(options) {
            ...
            vis.selectAll("rect").data(options.data).enter().append("svg:rect").attr("x", function(d, i) {
                return i * (w / options.data.length);
            }).attr("y", function(d) {
                return y(d[1]);
            }).attr("width", w / options.data.length - options.barPadding).attr("height", function(d) {
            ...
            vis.selectAll("text").data(options.data).enter().append("text").text(function(d) {
                return d[1];
            }).attr("x", function(d, i) {
                return i * (w / options.data.length);
            }).attr("y", function(d) {
                return y(d[1]) - 5;
            }).attr("font-family", "sans-serif").attr("font-size", "8px").attr("fill", "black").attr("title", function(d) {
                return d[0];
            });
        }
    });
