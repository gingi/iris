(function () {
    ...
    Iris.Renderer.create({
        ...
        render: function(settings) {
            ...
            rules.append("line").attr("x1", x).attr("x2", x).attr("y1", 0).attr("y2", options.height - 1);

            rules.append("line").attr("class", function(d) {
                return d ? null : "axis";
            }).attr("y1", y).attr("y2", y).attr("x1", 0).attr("x2", options.width + 1);

            rules.append("text").attr("x", x).attr("y", options.height + 3).attr("dy", ".71em").attr("text-anchor", "middle").attr("class", "axis-text").text(x.tickFormat(options.ticksx));

            rules.append("text").attr("y", y).attr("x", -3).attr("dy", ".35em").attr("text-anchor", "end").attr("class", "axis-text").text(y.tickFormat(options.ticksy));

            vis.selectAll("path").data(options.data).enter().append("svg:path").attr("d", d3.svg.line().x(function(d) {
                return x(d[0]);
            }).y(function(d) {
                return y(d[1]);
