(function( jQuery ) {
	var schema = {
		properties: {
			...
			strokeStyle: {type: "string", format: "color", required: true},
			mincolor: {type: 'array', items: {type: 'integer'}},
			maxcolor: {type: 'array', items: {type: 'integer'}},
			glyph: {type: 'string'},
			radius: {type: 'number'},
			data: {
				required: true,
	...
	var methods = {
		...
		render : function ( settings ) {
			...
			var colorRange = [
				this.options.maxcolor[0] - this.options.mincolor[0],
				this.options.maxcolor[1] - this.options.mincolor[1],
				this.options.maxcolor[2] - this.options.mincolor[2],
			];
			var min = new Array();
			var max = new Array();
	...
	jQuery.fn.RendererScatterplot = function( method ) {
		if ( methods[method] ) {
			return methods[ method ].apply( this, Array.prototype.slice.call( arguments, 1 ));
		} else {
			jQuery.error( 'Method ' +  method + ' does not exist on jQuery.RendererScatterplot' );
		}
	};
})( jQuery );
