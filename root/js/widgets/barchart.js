...
BarChart.prototype.draw_barChart = function(width){
    ...
    var max_width = document.getElementById(divId).clientWidth - sidePad - xpad;
...
//        .range([0,(hGraph)]);
    ...
    .append("svg")
    .attr("class", "chart")
    .attr("width", document.getElementById(divId).clientWidth - sidePad)
    ...
    if (w >=15){
        ...
        .attr("y",
        function(d, i) {
            return (h  + (h * (Math.floor(i/rowMax))) - (h-hGraph-5));
        })
        .attr("text-anchor", "start") // text-align: right
        .attr("transform", function(d, i) { return "rotate(90 " + ((i % rowMax) * w +xpad) + ","
         + (h  + (h * (Math.floor(i/rowMax))) - (h-hGraph-5)) + ")"; })
...
*/
		...
		this.svg.selectAll("axis labels")
		.data(y.ticks(5))
		.enter().append("text")
		.attr("x",xpad-5)
		.attr("y",y)
