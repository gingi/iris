...
function canvas_to_chr(a,b) {
	...
	var a_chr=0;
	var b_chr=0;
	var gutters = (chr_lengths.length + 1)*XGUTTER;
	var nt2px = (ctxi.canvas.width - gutters)/total_len;
	var a_pos, b_pos;
	for(var i=0;i<chr_lengths.length;i++) {
		...
		if (b >= offset && b <= offset + chr_xsize + XGUTTER) {
			b_chr = i+1;
...
function do_scatter(ctx,study,chr,offset,xsize,ysize,chr_length) {
	$.getJSON("/data/scatter/GWAS/"+study+"/"+chr+"/"+Math.floor(xsize/sc)+"/"+Math.floor(ysize/sc)+"/"+0+"/"+chr_length+"/"+0+"/"+global_max,
		function(json) {
...
function do_scatter_dots(ctx,study,chr,offset,xsize,ysize,chr_length) {
	$.getJSON("/data/scatter/GWAS/nobinning/"+study+"/"+chr,
		function(json) {
			var xrange = chr_length;
			var yrange = global_max;
			var xfactor = xsize/xrange;
...
function ev_mouseup (ev) {
	if (tool.started) {
		...
		var msg = "selected rectangle (" + x1 + "," + y1 + "," + x2 + "," + y2 + ") - score range=["+score_b+","+score_a+"] chr_range="+chr_range_string;
        manager.notify(containerNode.id, [score_b, score_a, chr_range_string]);
		tool.started = false;
        ctxi.clearRect(0,0,ctxi.canvas.width,ctxi.canvas.height);
	}
}
